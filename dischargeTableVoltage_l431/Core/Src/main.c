...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  //The L4 ADC must be calibrated before it is enabled for the first time.
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		if(conversionComplete){
			...
			//The mux is read on the ADC1 injected channel now that there is only one ADC.
			for (int i = 0; i <= 7; i++)
				dischargeLines[i] = readMuxChannel(i, &hadc1);
